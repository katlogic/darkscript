...
(function() {
  var BOM, BOOL, CALLABLE, CODE, COFFEE_ALIASES, COFFEE_ALIAS_MAP, COFFEE_KEYWORDS, COMMENT, COMPARE, COMPOUND_ASSIGN, HEREDOC, HEREDOC_ILLEGAL, HEREDOC_INDENT, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDENTABLE_CLOSERS, INDEXABLE, INVERSES, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LINE_BREAK, LINE_CONTINUER, LOGIC, Lexer, MATH, MULTILINER, MULTI_DENT, NOT_REGEX, NOT_SPACED_REGEX, NUMBER, OPERATOR, REGEX, REGEXP_MATCHES, RELATION, RESERVED, Rewriter, SHIFT, SIMPLESTR, STRICT_PROSCRIBED, TRAILING_SPACES, UNARY, UNARY_MATH, WHITESPACE, compact, invertLiterate, key, last, repeat, starts, throwSyntaxError, _ref, _ref1,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  _ref = require('./rewriter'), Rewriter = _ref.Rewriter, INVERSES = _ref.INVERSES;

  _ref1 = require('./helpers'), starts = _ref1.starts, compact = _ref1.compact, last = _ref1.last, repeat = _ref1.repeat, invertLiterate = _ref1.invertLiterate, throwSyntaxError = _ref1.throwSyntaxError;
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.tokenize = function(code, opts) {
      ...
      while (this.chunk = code.slice(i)) {
        consumed = this.identifierToken() || this.regexpToken() || this.commentToken() || this.whitespaceToken() || this.lineToken() || this.heredocToken() || this.stringToken() || this.numberToken() || this.regexToken() || this.jsToken() || this.literalToken();
    ...
    Lexer.prototype.literalToken = function() {
      ...
      if (value === ';') {
        this.seenFor = false;
        tag = 'TERMINATOR';
      } else if (value === '!' && prev && !prev.spaced && (_ref4 = prev[0], __indexOf.call(CALLABLE, _ref4) >= 0)) {
        tag = 'ASYNC';
    ...
    Lexer.prototype.regexpToken = function() {
      var id, input, m, tagToken, _;
      if (!(m = REGEXP_MATCHES.exec(this.chunk))) {
        return 0;
      }
      input = m[0], _ = m[1], id = m[2];
      id = (function() {
        switch (id) {
          case '~':
            return '__matches';
      ...
      tagToken = this.token('IDENTIFIER', id, 0, id.length);
      return input.length;
    };
    ...
    Lexer.prototype.getLineAndColumnFromChunk = function(offset) {
      var column, lineCount, lines, pos, string;
      if (offset === 0) {
