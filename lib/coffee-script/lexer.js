(function() {
  var ASYNC_END_IDENTIFIER, BOOL, CALLABLE, CODE, COFFEE_ALIASES, COFFEE_ALIAS_MAP, COFFEE_KEYWORDS, COMMENT, COMPARE, COMPOUND_ASSIGN, HEREDOC, HEREDOC_ILLEGAL, HEREDOC_INDENT, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDEXABLE, INVERSES, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LINE_BREAK, LINE_CONTINUER, LOGIC, Lexer, MATH, MULTILINER, MULTI_DENT, NOT_REGEX, NOT_SPACED_REGEX, NUMBER, OPERATOR, REDO_IDENTIFIER, REGEX, REGEX_MAGIC_IDENTIFIER, RELATION, RESERVED, Rewriter, SHIFT, SIMPLESTR, SYMBOLSTR, TRAILING_SPACES, UNARY, WHITESPACE, compact, count, key, last, starts, _ref, _ref2,
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.tokenize = function(code, opts) {
      ...
      while (this.chunk = code.slice(i)) {
        i += this.identifierToken() || this.asyncEndToken() || this.commentToken() || this.whitespaceToken() || this.lineToken() || this.heredocToken() || this.stringToken() || this.numberToken() || this.regexToken() || this.jsToken() || this.literalToken();
      }
    ...
    Lexer.prototype.identifierToken = function() {
      var colon, forcedIdentifier, id, index, input, match, matches, prev, submatch, tag, _ref3, _ref4;
    ...
    Lexer.prototype.asyncEndToken = function() {
      var m;
      if (this.tag() === 'TERMINATOR' && (m = this.chunk.match(ASYNC_END_IDENTIFIER))) {
        this.token('IDENTIFIER', '__async_end');
  ...
  RESERVED = ['case', 'default', 'function', 'var', 'void', 'with', 'const', 'let', 'enum', 'export', 'import', 'native', '__hasProp', '__extends', '__slice', '__bind', '__indexOf', '__matches', '__async_end'];

  JS_FORBIDDEN = JS_KEYWORDS.concat(RESERVED);

  exports.RESERVED = RESERVED.concat(JS_KEYWORDS).concat(COFFEE_KEYWORDS);

  IDENTIFIER = /^(\\[&~1-9]|[$A-Za-z_\x7f-\uffff][$\w\x7f-\uffff!]*)(:(?!:)|[^\n\S]*:(?!:|\w|[\x7f-\uffff]))?/;
