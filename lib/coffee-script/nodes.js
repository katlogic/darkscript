...
(function() {
  ...
  exports.While = While = (function(_super) {
    ...
    While.prototype.asyncCompileNode = function(o, info) {
      var answer, blocks, body_fn, call_body, code, done, done_body, done_fn, flow, ifPart, name, names, ret, _i, _len, _ref4;
      if (this.flow) {
        o.flows.push(this.flow);
      }
      flow = o.flows.last();
      ...
      body_fn = new Assign(new Literal(names.body), code = new Code([], new Block([ifPart = new If(this.condition, info.body).addElse(ret = new Call(new Literal(done)))]), 'boundfunc', {
        next: names.body,
        "return": flow["return"],
        "break": done,
        "continue": names.body
  ...
  exports.For = For = (function(_super) {
    ...
    For.prototype.asyncCompileNode = function(o, info) {
      var answer, blocks, body_fn, call_body, code, done, done_fn, flow, ifPart, name, names, ret, step_fn, _i, _len, _ref4;
      if (this.flow) {
        o.flows.push(this.flow);
      }
      flow = o.flows.last();
      ...
      if (flow.next) {
        ...
        done_fn = new Assign(new Literal(names.done), new Code([], new Block(), 'boundfunc', flow));
        blocks.push(done_fn);
      }
      blocks.push(new Literal(info.initPart));
      step_fn = new Assign(new Literal(names.step), new Code([], new Block([new Literal(info.stepPart), ret = new Call(new Literal(names.body))])));
      ret.omit_return = true;
      blocks.push(step_fn);
      if (info.varPart) {
        info.body.unshift(new Literal(info.varPart.trim().slice(0, -1)));
      }
      body_fn = new Assign(new Literal(names.body), code = new Code([], new Block([ifPart = new If(new Literal(info.condPart), info.body).addElse(ret = new Call(new Literal(done)))]), 'boundfunc', {
        next: names.step,
        "return": flow["return"],
        "break": done,
        "continue": names.step
  ...
  exports.Await = Await = (function(_super) {
    ...
    Await.prototype.children = ['call', 'next'];
    ...
    Await.prototype.makeReturn = function(res) {
      return this;
    };

    Await.prototype.get_defer = function() {
