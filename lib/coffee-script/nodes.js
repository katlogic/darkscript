...
(function() {
  var $verbose, Access, Arr, Assign, Async, AsyncCall, AsyncValue, Base, Block, Call, Class, Closure, Code, CodeFragment, Comment, Defer, Existence, Extends, Flows, For, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Next, Obj, Op, Param, Parens, RESERVED, Range, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, addLocationDataFn, compact, del, ends, extend, flatten, fragmentsToText, last, locationDataToString, merge, multident, p, puts, some, starts, throwSyntaxError, uid, unfoldSoak, utility, _ref, _ref1, _ref2, _ref3,
  ...
  exports.Base = Base = (function() {
    ...
    Base.prototype.replaceAsyncStatement = function() {
      return this.eachChild(function(child) {
        return child.replaceAsyncStatement();
      });
    };
    ...
    */
    ...
    Base.move_fn = function(dest, node, cross) {
      var call, code, code_body;
      if (cross == null) {
        cross = true;
      }
  ...
  exports.Block = Block = (function(_super) {
    ...
    Block.prototype.makeReturn = function(res) {
      ...
      while (len--) {
        ...
        if (res && expr instanceof Async) {
          ...
          ret.generated = true;
          this.expressions.splice(len + 1, 0, ret);
          break;
        }
    ...
    Block.prototype.asyncCompileNode = function(o) {
      var flow, index, next, next_code, next_name, node, _i, _len, _ref2, _ref3;
      flow = extend({}, o.flows.last());
      _ref2 = this.expressions;
      for (index = _i = 0, _len = _ref2.length; _i < _len; index = ++_i) {
        node = _ref2[index];
        if (node.async) {
          node.next_code = this.pop_next_code(flow, index);
          break;
        } else if (node.async && ((_ref3 = node.constructor.name) === 'For' || _ref3 === 'If' || _ref3 === 'While')) {
          next_code = this.pop_next_code(flow, index);
          if (next_code.body.isEmpty()) {
            ...
            node.flow = extend(flow, {
              next: next_name
            });
            this.expressions.splice(index, 0, next);
          }
          break;
        }
      }
    ...
    Block.prototype.replaceAsyncStatement = function() {
      var call, code, expr, index, next_code, node, _i, _len, _ref2;
      return;
      _ref2 = this.expressions;
      for (index = _i = 0, _len = _ref2.length; _i < _len; index = ++_i) {
        node = _ref2[index];
        if (node.async && node instanceof Assign && !(node.value instanceof Code)) {
          ...
          node.async = false;
          next_code = new Code([], new Block(this.expressions.splice(index + 1)), 'boundfunc');
          next_code.async = true;
          next_code.body.unshift(node);
          call = new Call(code = new Code([new Param(new Literal('autocb'))], new Block([expr]), 'boundfunc'), [next_code]);
          code.async = true;
          this.expressions.splice(index, 1, call);
  ...
  exports.Literal = Literal = (function(_super) {
    ...
    Literal.prototype.move = function(dest) {
      return Base.move(dest, this).base;
    };
  ...
  exports.Return = Return = (function(_super) {
    ...
    Return.prototype.move = function(dest) {
      var expr, idx, _i, _len, _ref4;
      _ref4 = this.expression;
      for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
        expr = _ref4[idx];
        if (expr.async) {
          this.expression[idx] = expr.move(dest);
        }
      }
      this.async = false;
      return this;
  ...
  exports.Value = Value = (function(_super) {
    ...
    Value.prototype.move = function(dest) {
      var idx, prop, _i, _len, _ref4;
      this.base = this.base.move(dest);
      _ref4 = this.properties;
      for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
        prop = _ref4[idx];
        if (prop.async) {
          this.properties[idx] = prop.move(dest);
        }
      }
      this.async = false;
      return this;
  ...
  exports.Call = Call = (function(_super) {
    ...
    Call.prototype.move = function(dest) {
      var arg, idx, _i, _len, _ref4;
      _ref4 = this.args;
      for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
        arg = _ref4[idx];
        if (arg.async) {
          this.args[idx] = arg.move(dest);
        }
      }
  ...
  exports.Obj = Obj = (function(_super) {
    ...
    Obj.prototype.move = function(dest) {
      ...
      while (--len) {
        if (this.properties[len].async) {
          break;
        }
      }
      for (idx = _i = 0; 0 <= len ? _i <= len : _i >= len; idx = 0 <= len ? ++_i : --_i) {
        obj = this.properties[idx];
        this.properties[idx].value = obj.value.move(dest);
        this.properties[idx].async = false;
      }
      this.async = false;
      return this;
    };
  ...
  exports.Arr = Arr = (function(_super) {
    ...
    Arr.prototype.move = function(dest) {
      var idx, obj, _i, _len, _ref4;
      _ref4 = this.objects;
      for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
        obj = _ref4[idx];
        if (obj.async) {
          this.objects[idx] = obj.move(dest);
        }
      }
      this.async = false;
      return this;
  ...
  exports.Assign = Assign = (function(_super) {
    ...
    Assign.prototype.transform = function(next) {
      var assigns, call, idx, obj, params, v;
      v = this.value;
      if (v instanceof AsyncCall) {
        call = v;
        this.value = new Value(new Literal("arguments"));
        if (!(this.variable.base instanceof Arr)) {
          this.variable = new Value(new Arr([this.variable]));
        }
        this.async = false;
        if (!this.moved) {
          assigns = (function() {
            var _i, _len, _ref4, _results;
            _ref4 = this.variable.base.objects;
            _results = [];
            for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
              obj = _ref4[idx];
              _results.push(new Assign(obj, new Value(this.value.base, [new Index(new Value(new Literal(idx)))])));
            }
            return _results;
          }).call(this);
          next.unshift.apply(next, assigns);
        } else {
          params = (function() {
            var _i, _len, _ref4, _results;
            _ref4 = this.variable.base.objects;
            ...
            for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
              obj = _ref4[idx];
              _results.push(new Param(obj.base));
            }
            return _results;
          }).call(this);
        }
        return call.transform(next, params);
      } else {
    ...
    Assign.prototype.move = function(dest) {
      ...
      if (this.value instanceof AsyncCall) {
        this.value = this.value.move_args(dest);
        return this;
      }
      this.value = Base.move_fn(dest, this.value);
      assign = dest.pop();
  ...
  exports.Code = Code = (function(_super) {
    ...
    Code.prototype.compileNode = function(o) {
      var answer, code, exprs, i, idt, lit, param, params, ref, splats, uniqs, val, wasEmpty, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref4, _ref5, _ref6, _ref7, _ref8;
  ...
  exports.While = While = (function(_super) {
    ...
    While.prototype.compileNode = function(o) {
      ...
      info = {};
      this.returns || (this.returns = this.return_results);
      if (!this.async) {
        o.indent += TAB;
      }
      set = '';
  ...
  exports.Op = Op = (function(_super) {
    ...
    Op.prototype.move_or = function(dest) {
      var body, first, second;
      body = [];
      first = this.first.move(body);
      body.push(new If(first, new Return(first)));
      second = this.second.move(body);
      body.push(new Return([second]));
      return Base.move_fn(dest, new Block(body), false);
    };

    Op.prototype.move_and = function(dest) {
      var body, first;
      body = [];
      first = this.first.move(body);
      body.push(new If(first.invert(), new Return(first)));
      body.push(new Return([second]));
      return Base.move_fn(dest, new Block(body), false);
    };
  ...
  exports.If = If = (function(_super) {
    ...
    If.prototype.asyncCompileNode = function(o) {
      var body, flow, ret, _i, _len, _ref4, _ref5;
      flow = o.flows.last();
      if (flow.next && !this.elseBody) {
    ...
    If.prototype.transform = function(next) {
      var _ref4, _ref5;
      this.next = next;
      if ((_ref4 = this.body) != null) {
        _ref4.transform();
      }
      if ((_ref5 = this.elseBody) != null) {
        _ref5.transform();
      }
      return [this];
    };

    If.prototype.move = function(dest) {
      var _ref4;
  ...
  exports.Async = Async = (function(_super) {
    __extends(Async, _super);

    function Async(value, args, assign) {
      this.value = value;
      this.args = args;
      this.assign = assign;
      this.async = true;
    ...
    Async.prototype.children = ['value', 'args', 'assign'];

    Async.prototype.compileNode = function(o) {
      var assign, assigns, call, def, next_code, pos, _i, _len, _ref4;
      return "";
      call = this.call, next_code = this.next_code;
      call.omit_return = true;
      _ref4 = this.get_defer(), def = _ref4[0], pos = _ref4[1];
    ...
    Async.prototype.makeReturn = function(res) {
      return this;
    };

    Async.prototype.get_defer = function() {
      var arg, pos, _i, _len, _ref4;
      _ref4 = this.call.args;
      for (pos = _i = 0, _len = _ref4.length; _i < _len; pos = ++_i) {
        arg = _ref4[pos];
        if (arg instanceof Defer) {
          return [arg, pos];
        }
      }
      return this.error("cannot find defer");
    };
  ...
  exports.AsyncValue = AsyncValue = (function(_super) {
    __extends(AsyncValue, _super);
    ...
    AsyncValue.prototype.children = ['value'];
  ...
  exports.AsyncCall = AsyncCall = (function(_super) {
    ...
    AsyncCall.prototype.transform = function(next, params) {
      var code, node;
      if (params == null) {
        params = [];
      }
  ...
  exports.Next = Next = (function(_super) {
    __extends(Next, _super);

    function Next(value) {
      this.value = value;
      this.value = Block.wrap(this.value);
    }

    Next.prototype.children = ['value'];

    Next.prototype.unshift = function() {
      var _ref4;
      return (_ref4 = this.value.expressions).unshift.apply(_ref4, arguments);
    };

    Next.prototype.push = function() {
      var _ref4;
      return (_ref4 = this.value.expressions).push.apply(_ref4, arguments);
    };

    Next.prototype.pop = function() {
      return this.value.expressions.pop();
    };
  ...
  exports.Defer = Defer = (function(_super) {
    ...
    Defer.prototype.get_vars = function() {
      var arg, v, _i, _len, _ref4, _results;
      _ref4 = this.args;
      _results = [];
      for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
        arg = _ref4[_i];
        if (v = arg.unwrap().value) {
          _results.push(v);
        }
      }
      return _results;
    ...
    Defer.prototype.get_assigns = function() {
      var arg, idx, value, _i, _len, _ref4, _results;
      _ref4 = this.args;
      _results = [];
      for (idx = _i = 0, _len = _ref4.length; _i < _len; idx = ++_i) {
        arg = _ref4[idx];
        value = new Value(new Literal("arguments[" + idx + "]"));
        _results.push(new Assign(arg, value));
      }
      return _results;
    };
  ...
  Flows = (function() {
    ...
    Flows.prototype.clone = function() {
      return this.push(extend({}, last(this.flows)));
    };
