...
(function() {
  var Access, Arr, Assign, Await, Base, Block, Call, Class, Closure, Code, CodeFragment, Comment, Defer, Existence, Extends, For, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Next, NextPlaceholder, Obj, Op, Param, Parens, RESERVED, Range, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, addLocationDataFn, compact, del, ends, extend, flatten, fragmentsToText, last, locationDataToString, merge, multident, p, puts, some, starts, throwSyntaxError, unfoldSoak, utility, _ref, _ref1, _ref2, _ref3,
  ...
  exports.Base = Base = (function() {
    ...
    Base.prototype.setVars = function(scopes) {
      return this.traverse({
        from_parent: function(self, parent) {
          var scope;
  ...
  exports.Block = Block = (function(_super) {
    ...
    Block.prototype.makeReturn = function(res) {
      ...
      if (len === 0 && this.autocb) {
        ret = new Return([], this.autocb);
        ret.last_expr = true;
    ...
    Block.prototype.compileNode = function(o) {
      var answer, compiledNodes, fragments, index, node, nodes, top, v, _i, _j, _k, _len, _len1, _len2, _ref2, _ref3, _ref4;
      this.tab = o.indent;
    ...
    Block.prototype.sync = function() {
      var forward, idx, next, node, _i, _len, _ref2, _ref3, _ref4, _ref5;
      _ref2 = this.expressions;
      for (idx = _i = 0, _len = _ref2.length; _i < _len; idx = ++_i) {
        node = _ref2[idx];
        if (node instanceof Await) {
          if ((_ref3 = node.next) == null) {
            node.next = this.pop_next(idx);
          }
          break;
        } else if (node.async && ((_ref4 = node.constructor.name) === 'For' || _ref4 === 'If' || _ref4 === 'While')) {
          next = this.pop_next(idx);
          if (next.isEmpty()) {
            next = null;
          } else if (forward = next.forward()) {
            next = forward;
          } else {
            this.expressions.splice(idx, 0, next);
          }
          if ((_ref5 = node.next) == null) {
            node.next = next;
          }
          node.sync(next);
          break;
    ...
    Block.prototype.pop_next = function(idx) {
      var next_block, next_code;
      next_block = Block.wrap(this.expressions.splice(idx + 1));
      next_code = new Code([], next_block, 'boundfunc');
      next_code.autocb = this.autocb;
      next_code.setFlags();
      next_code.sync();
      return new Next(next_code);
    };
  ...
  exports.Return = Return = (function(_super) {
    ...
    Return.prototype.compileNode = function(o) {
      ...
      if (this.autocb) {
        ...
        expr = new Call(cb, this.expression);
        expr.omit_return = this.last_expr;
      } else if (this.expression.length > 1) {
        this.error("only autocb can return multiple value");
      } else {
  ...
  exports.Code = Code = (function(_super) {
    ...
    function Code(params, body, tag) {
      ...
      for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
        param = _ref4[_i];
        if (!((param != null ? (_ref5 = param.name) != null ? _ref5.value : void 0 : void 0) === 'autocb')) {
          continue;
        }
        this.autocb = true;
        break;
    ...
    Code.prototype.compileNode = function(o) {
      ...
      if (!((wasEmpty || this.noReturn) && !this.autocb)) {
  ...
  exports.For = For = (function(_super) {
    ...
    For.prototype.compileNode = function(o) {
      var body, bodyFragments, compare, compareDown, declare, declareDown, defPart, defPartFragments, down, forPartFragments, guardPart, idt1, increment, index, info, ivar, kvar, kvarAssign, lastJumps, lvar, name, namePart, ref, resultPart, returnResult, rvar, scope, source, step, stepNum, stepVar, svar, varPart, _ref4, _ref5;
    ...
    For.prototype.asyncCompileNode = function(o, info) {
      var block, body, body_code, body_name, call, cond_check, init, init_code, init_name, next_name, ret, scope, step, step_code, step_name, vars, _ref4, _ref5;
      scope = o.scope;
      body_name = new Value(new Literal(scope.freeVariable('body')));
      ...
      info.body.setFlags();
      info.body.sync(this);
      cond_check = new If(new Value(new Literal(info.cond)), info.body);
      if (this.next != null) {
        next_name = this.next.name;
      } else {
        next_name = o.next_name;
      ...
      vars = (_ref4 = (_ref5 = info.vars) != null ? _ref5.trim().slice(0, -1) : void 0) != null ? _ref4 : "";
      body_code = new Code([new Param(new Literal('autocb'))], new Block([new Literal(vars), cond_check]), 'boundfunc');
      body = new Assign(body_name, body_code);
      init_code = new Code([], new Block([new Literal(info.init)]), 'bountfunc');
      init_code.noReturn = true;
      init = new Assign(init_name, init_code);
      step_code = new Code([], new Block([new Literal(info.step), new Call(body_name, [step_name])]));
      step_code.noReturn = true;
      step = new Assign(step_name, step_code);
      block = Block.wrap([new Literal(info.init), step, body, new Call(body_name, [step_name])]);
      o.next_name = next_name;
      return block.compileNode(o);
    };
  ...
  exports.If = If = (function(_super) {
    ...
    If.prototype.asyncPrecompile = function() {
      var body, call, _i, _len, _ref4, _ref5, _ref6;
      if (this.autocb || name) {
        if ((_ref4 = this.elseBody) == null) {
          this.elseBody = new Block([]);
        }
        this.elseBody.autocb = this.autocb;
      }
      if ((_ref5 = this.elseBody) != null ? _ref5.async : void 0) {
        this.elseBody.isChain = false;
      }
      if (name) {
      ...
      return If.__super__.asyncPrecompile.apply(this, arguments);
    };

    If.prototype.sync = function(next) {
      var placeholder, _ref4, _ref5;
      if (this.autocb && this.async || next) {
        if ((_ref4 = this.elseBody) == null) {
          this.elseBody = new Block([]);
        }
        this.elseBody.autocb = this.autocb;
      ...
      if (!next) {
        return If.__super__.sync.apply(this, arguments);
      }
      ...
      this.elseBody.push(placeholder);
      return If.__super__.sync.apply(this, arguments);
    };
  ...
  exports.Await = Await = (function(_super) {
    ...
    Await.prototype.compileNode = function(o) {
      var assign, assigns, call, def, next, pos, _i, _len, _ref4;
      next = this.next;
      call = this.call;
      call.omit_return = true;
      next = next.code;
    ...
    Await.prototype.sync = function(next) {
      this.next = next;
      return this;
    };
  ...
  exports.Next = Next = (function(_super) {
    ...
    function Next(code) {
      this.code = code;
      this.autocb = this.code.autocb;
      if (this.isEmpty() && this.autocb) {
        this.name = new Literal('autocb');
      }
    }

    Next.prototype.children = ['code'];

    Next.prototype.isEmpty = function() {
      var _ref4;
      return (_ref4 = this.code.body) != null ? _ref4.isEmpty() : void 0;
    };

    Next.prototype.forward = function() {
      var placeholder, _ref4, _ref5;
      if ((placeholder = (_ref4 = this.code.body) != null ? (_ref5 = _ref4.expressions) != null ? _ref5[0] : void 0 : void 0) instanceof NextPlaceholder) {
        return placeholder.next;
      }
      return null;
    };

    Next.prototype.assign = function(o) {
      if (this.name) {
        return new Block();
      }
      this.name = new Literal(o.scope.freeVariable('fn'));
      if (o.next_name) {
        this.code.body.push(new Call(o.next_name));
      }
      return new Assign(this.name, this.code);
    };

    Next.prototype.placeholder = function() {
      return new NextPlaceholder(this);
    };
  ...
  exports.NextPlaceholder = NextPlaceholder = (function(_super) {
    __extends(NextPlaceholder, _super);

    function NextPlaceholder(next) {
      this.next = next;
      this.omit_return = true;
      this.autocb = this.next.autocb;
    }

    NextPlaceholder.prototype.compileNode = function(o) {
      var rt;
      rt = new Call(this.next.name, []).compileNode(o);
      return rt;
    };
