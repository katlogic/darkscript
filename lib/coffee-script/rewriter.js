(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.rewriteAsynchronous = function() {
      var asyncParams, asyncTokens, closeCallback, equalToken, i, id, indents, insertCallbackStart, isAsyncToken, last, len, length, line, m, pairs, pushAsyncParams, pushToken, pushTokens, stacks, tag, tempTokens, token, updateAsyncParams, _ref, _ref2, _ref3;
      asyncTokens = [];
      asyncParams = [];
      line = 0;
      i = 0;
      ...
      pushTokens = function(tokens) {
        ...
        for (_i = 0, _len = tokens.length; _i < _len; _i++) {
          token = tokens[_i];
          _results.push(asyncTokens.push(token));
        }
        return _results;
      };
      updateAsyncParams = function() {
        var last, tokens;
        tokens = asyncParams;
        last = tokens.length - 1;
        if (tokens.length >= 2 && tokens[0][0] === '[' && tokens[last][0] === ']') {
          tokens[0] = ['PARAM_START', '(', tokens[0][2]];
          return tokens[last] = ['PARAM_END', ')', tokens[last][2]];
        } else {
          tokens.unshift(['PARAM_START', '(', line]);
          return tokens.push(['PARAM_END', ')', line]);
        }
      };
      pushAsyncParams = function() {
        pushTokens(asyncParams);
        return asyncParams = [];
      };
      isAsyncToken = function(token) {
        return token && token[0] === 'IDENTIFIER' && token[1].match(/\!$/);
      };
      indents = [];
      closeCallback = function() {
        var len;
        if (last(indents) === 'CLOSE_CALLBACK') {
          len = asyncTokens.length;
          if (asyncTokens[len - 1] && asyncTokens[len - 1][0] === 'TERMINATOR') {
            asyncTokens.pop();
          }
      ...
      insertCallbackStart = function() {
        ...
        return indents.push('CLOSE_CALLBACK');
      };
      for (i = 0; 0 <= length ? i < length : i > length; 0 <= length ? i++ : i--) {
        token = this.tokens[i];
        tag = token[0], id = token[1], line = token[2];
        switch (tag) {
          case '[':
            pushAsyncParams();
            stacks = 0;
          ...
          case '=':
            ...
            while (true) {
              ...
              if (stacks.length === 0 && isAsyncToken(token)) {
                --i;
                tempTokens.pop();
                pushTokens(tempTokens);
                break;
              } else if ((_ref = token[0]) === '[' || _ref === '(' || _ref === '{') {
                stacks.push(token);
              } else if ((_ref2 = token[0]) === ']' || _ref2 === ')' || _ref2 === '}') {
                stacks.pop(token);
              } else if (stacks.length > 0) {} else if ((_ref3 = token[0]) === 'IDENTIFIER' || _ref3 === '.' || _ref3 === '?.' || _ref3 === '::' || _ref3 === '@') {} else {
                pushAsyncParams();
          ...
          case 'IDENTIFIER':
          case '@':
            if (token[0] === 'IDENTIFIER' && token[1] === '__async_end') {
              pushAsyncParams();
              while (closeCallback()) {
                continue;
              }
          ...
          case 'CALL_END':
            pushAsyncParams();
            if (last(indents) !== 'CALL_START') {
              throw new Error('CALL_END without CALL_START');
            }
            indents.pop();
            if (last(indents) === 'OPEN_CALLBACK') {
              insertCallbackStart();
            } else {
              pushToken(token);
            }
          ...
          case 'TERMINATOR':
            len = asyncTokens.length;
            if (asyncParams.length === 0 && asyncTokens[len - 1] && asyncTokens[len - 1][0] === 'INDENT') {} else {
              pushToken(token);
            }
            break;
          default:
