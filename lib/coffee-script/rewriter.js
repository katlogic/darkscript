...
(function() {
  var BALANCED_PAIRS, CALL_CLOSERS, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, SINGLE_CLOSERS, SINGLE_LINERS, left, rite, _i, _len, _ref,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __slice = [].slice;

  function generate(tag, value, origin) {
    var tok;
    tok = [tag, value];
    tok.generated = true;
    if (origin) {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.closeOpenCalls = function() {
      function condition(token, i) {
        var _ref;
        return ((_ref = token[0]) === ')' || _ref === 'CALL_END') || token[0] === 'OUTDENT' && this.tag(i - 1) === ')';
      };
      function action(token, i) {
    ...
    Rewriter.prototype.addImplicitBracesAndParens = function() {
      ...
      return this.scanTokens(function(token, i, tokens) {
        var newLine, nextTag, offset, prevTag, prevToken, s, sameLine, stackIdx, stackTag, startIdx, startsLine, tag, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
        tag = token[0];
        ...
        function inImplicitObject() {
          var _ref;
          return inImplicit() && ((_ref = stackTop()) != null ? _ref[0] : void 0) === '{';
        };
        function inImplicitControl() {
          var _ref;
          return inImplicit && ((_ref = stackTop()) != null ? _ref[0] : void 0) === 'CONTROL';
        };
        function startImplicitCall(j) {
          var idx;
          idx = j != null ? j : i;
          stack.push([
            '(', idx, {
        ...
        function endImplicitCall() {
          stack.pop();
          tokens.splice(i, 0, generate('CALL_END', ')'));
          return i += 1;
        };
        function startImplicitObject(j, startsLine) {
          var idx;
          if (startsLine == null) {
            startsLine = true;
          }
    ...
    Rewriter.prototype.rewriteAsync = function() {
      var dest, line, tag_async, tag_ce, tag_cs, token, tokens, _ref, _ref1;
      dest = [];
      tokens = this.tokens;
      while (token = tokens.shift()) {
        if (token[0] === 'IDENTIFIER' && token[1].slice(-1) === '!') {
          ...
          dest.push(token);
          dest.push(tag_async);
          if (((_ref = tokens[0]) != null ? _ref[0] : void 0) !== 'CALL_START') {
            dest.push(tag_cs);
            dest.push(tag_ce);
          }
        } else if (token[0] === 'ASYNC' && ((_ref1 = tokens[0]) != null ? _ref1[0] : void 0) !== 'CALL_START') {
          line = token[2];
          tag_cs = ['CALL_START', '(', line];
          tag_ce = ['CALL_END', ')', line];
          dest.push(token);
  ...
  IMPLICIT_CALL = ['IDENTIFIER', 'NUMBER', 'STRING', 'JS', 'REGEX', 'NEW', 'PARAM_START', 'CLASS', 'IF', 'TRY', 'SWITCH', 'THIS', 'BOOL', 'NULL', 'UNDEFINED', 'UNARY', 'YIELD', 'UNARY_MATH', 'SUPER', 'THROW', '@', '->', '=>', '[', '(', '{', '--', '++', 'DEFER', 'ASYNC', '!->', '!=>'];
