(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.toffeeHelpers = function() {
      var getTag, getToken, popBlockTokens, popBlockTokensUntil, popCaller, popTokensUntil, shiftBlockTokens, shiftBlockTokensUntil, shiftConditionBlock, shiftNextBlock, shiftParam, shiftTokensUntil, smartPush, tag,
        _this = this;
    ...
    Rewriter.prototype.rewriteAsyncCondition = function() {
      var async_tokens, call_func, condition, func_name, getAsync, line, name, next, next_tokens, old_tokens, outdent, shiftConditionBlock, shiftNextBlock, shiftTokensUntil, smartPush, stack, tag, token, _ref;
      stack = [];
      async_tokens = [];
      line = 0;
      getAsync = this.asyncFunctions().getAsync;
      _ref = this.toffeeHelpers(), shiftTokensUntil = _ref.shiftTokensUntil, shiftConditionBlock = _ref.shiftConditionBlock, shiftNextBlock = _ref.shiftNextBlock, tag = _ref.tag, smartPush = _ref.smartPush;
    ...
    Rewriter.prototype.cascade = function(force_complex) {
      ...
      while (token = this.tokens[TAG]) {
        if (token[0] === '{' && getTag(new_tokens) === '.') {
          ...
          if (complex) {
            smartPush(new_tokens, ['IDENTIFIER', '__cascade', lineno], ['CALL_START', '(', lineno], caller, [',', ',', lineno]);
