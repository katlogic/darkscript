...
(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.rewriteChannel = function() {
      var dest, ident, line, t, tag_dot, tag_push, tag_shift, token, tokens, _ref;
      ident = ['IDENTIFIER', ',', '@', '.', '::'];
      dest = [];
      tokens = this.tokens;
      while (token = tokens.shift()) {
        if (token[0] === '<-') {
          ...
          if (t && t[0] === 'IDENTIFIER') {
            ...
            while ((token = tokens[0]) && (_ref = token[0], __indexOf.call(ident, _ref) >= 0)) {
    ...
    Rewriter.prototype.rewriteAsync = function() {
      var caller, dest, end, ident, level, line, num, params, t, tag_await, tag_ce, tag_comma, tag_cs, tag_defer, token, tokens, _i, _j, _len, _len1, _ref, _ref1;
      ident = ['IDENTIFIER', ',', '@', '.', '::'];
      dest = [];
      tokens = this.tokens;
      while (token = tokens.shift()) {
        if (token[0] === 'IDENTIFIER' && token[1].slice(-1) === '!') {
          ...
          caller = [token];
          params = [];
          while ((t = dest.pop()) && (_ref = t[0], __indexOf.call(ident, _ref) >= 0)) {
            caller.unshift(t);
          }
          if (t && t[0] === '=') {
            while ((t = dest.pop()) && (_ref1 = t[0], __indexOf.call(ident, _ref1) >= 0)) {
              params.unshift(t);
            }
          }
          if (t) dest.push(t);
          dest.push(tag_await);
          for (_i = 0, _len = caller.length; _i < _len; _i++) {
            t = caller[_i];
            dest.push(t);
          }
          num = 0;
  ...
  IMPLICIT_CALL = ['IDENTIFIER', 'NUMBER', 'STRING', 'JS', 'REGEX', 'NEW', 'PARAM_START', 'CLASS', 'IF', 'TRY', 'SWITCH', 'THIS', 'BOOL', 'NULL', 'UNDEFINED', 'UNARY', 'SUPER', 'DEFER', '@', '->', '=>', '[', '(', '{', '--', '++'];
