(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.rewriteAsynchronous = function() {
      var IDENT, LINE, PARENS_END, PARENS_START, TAG, VALUE, async_tokens, caller, closeCallback, getAsync, getTag, getToken, line, name, openCallback, params, popCaller, popParams, pushTokens, raise, stack, status, token, _ref;
      var _this = this;
      TAG = 0;
      VALUE = 1;
      LINE = 2;
      ...
      popCaller = function() {
        ...
        while (token = getToken()) {
          ...
          if (PARENS_END[tag]) {
            ...
            --level;
          } else if (IDENT[tag]) {} else if (level > 0) {} else {
            break;
          }
          async_tokens.pop();
      ...
      openCallback = function(params) {
        ...
        return async_tokens.push(['INDENT', 2, line]);
      };
      ...
      while (token = this.tokens.shift()) {
        ...
        if (name = getAsync(token)) {
          ...
          if (token[TAG] === 'IDENTIFIER' && token[VALUE] === '__async_end') {
            token[TAG] = 'ASYNC_END';
          }
          if ((_ref = token[TAG]) === 'CALL_END' || _ref === 'OUTDENT' || _ref === 'ASYNC_END') {
            while (closeCallback()) {
              continue;
            }
          }
          switch (token[TAG]) {
            ...
            case 'CALL_END':
              ...
              break;
            case 'OUTDENT':
              if (stack.pop() !== 'INDENT') raise("Unexpected 'OUTDENT'");
              async_tokens.push(token);
              break;
            case 'TERMINATOR':
