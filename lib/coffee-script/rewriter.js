...
(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.toffeeHelpers = function() {
      ...
      smartPush = function() {
        var args, dest, token, tokens, _i, _j, _len, _len1;
    ...
    Rewriter.prototype.asyncFunctions = function(stack, async_tokens) {
      ...
      openCallback = function(params) {
        var assignment, async_id, comma, i, ident, ident_name, is_ident, len, level, new_ident, param, param_blocks, push_param, replacement, replacements, tag, _i, _j, _k, _len, _len1, _ref;
        ...
        for (_k = 0, _len1 = replacements.length; _k < _len1; _k++) {
    ...
    Rewriter.prototype.rewriteAsyncCondition = function() {
      ...
      while (token = this.tokens.shift()) {
        ...
        if ((name = getAsync(token)) && (name === 'if' || name === 'unless')) {
          ...
          if (tag(next, 0) === 'TERMINATOR') {
            next.shift();
          }
          if (tag(next, -1) === 'TERMINATOR') {
            next.pop();
          }
    ...
    Rewriter.prototype.new_caller_id = function() {
      if (this.caller_id_num == null) {
        this.caller_id_num = 0;
      }
      return "_asid" + this.caller_id_num++;
    };

    Rewriter.prototype.extend = function(force_complex) {
      var LINE, TAG, VALUE, caller, colon, comma, complex, func_params, getTag, getToken, key, lineno, new_caller, new_params, new_tokens, old_tokens, param, params, popBlockTokensUntil, popCaller, shiftBlockTokens, shiftNextBlock, shiftParam, smartPush, token, value, _i, _j, _len, _len1, _ref, _ref1;
      if (force_complex == null) {
        force_complex = false;
      }
      TAG = 0;
      VALUE = 1;
      ...
      while (token = this.tokens[0]) {
        if (token[0] === '{' && getTag(new_tokens) === '.') {
          ...
          if (force_complex && new_tokens.length === 0) {
            complex = true;
          } else if (new_tokens.length === 0 || ((_ref1 = getTag(new_tokens)) === 'TERMINATOR' || _ref1 === 'INDENT')) {
            complex = false;
          } else {
            complex = true;
          }
          ...
          if (complex) {
            ...
            for (_j = 0, _len1 = new_params.length; _j < _len1; _j++) {
              param = new_params[_j];
              key = param[0], value = param[1];
              lineno = key[LINE];
