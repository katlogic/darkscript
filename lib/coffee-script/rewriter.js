(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.rewriteAsynchronous = function() {
      var asyncParams, asyncTokens, closeCallback, i, id, indent, indents, isAsyncToken, length, line, pushAsyncParams, pushToken, pushTokens, tag, token, updateAsyncParams;
      asyncTokens = [];
      asyncParams = [];
      line = 0;
      i = 0;
      ...
      updateAsyncParams = function() {
        ...
        if (tokens.length < 2) {
          ...
          return tokens[last][0] = ['PARAM_END', ')', tokens[last][2]];
        } else {
      ...
      for (i = 0; 0 <= length ? i < length : i > length; 0 <= length ? i++ : i--) {
        ...
        switch (tag) {
          case '[':
            pushAsyncParams();
            while (token[0] !== ']' && (token = this.tokens[++i])) {
              asyncTokens.push(token);
            }
            break;
          case '=':
            if (!isAsyncIdentifier(this.tokens[i + 1])) {
              pushAsyncParams();
              asyncTokens.push(token);
            }
            break;
          case 'IDENTIFIER':
            if (isAsyncIdentifier(token)) {
              updateAsyncParams();
              indents.push(['IN_ASYNC', asyncParams]);
              asyncParams = [];
            } else {
              pushToken(token);
            }
