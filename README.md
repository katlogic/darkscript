...
edge CoffeeScript (1.8.x) and is currently in experimental shape.
...
For the most common and simple case, ie to make async code look synchronous without breaking the flow, we can use async/await style from C# - there, .NET compiler translates those into a callbacks. We can do the same, as well keeping compatibility with JS callback APIs out of the box.
...
The `!` after function value denotes that code which follows will be actually a calback, equivalent of `await` keyword:
...
`async` keyword equvivalent is a function expecting callback as last argument, which gets called instead of exiting to proceed with the continuation.
...
assert(asyn_fun!(1) == 2)
```
This manual handling of callback-instead-of-return-value could get tiresome quickly, so there is syntactic sugar
for calling the callback in place of implicit and explicit returns:
...
```
The ```semantics``` of return change only with magical last argument named ```autocb```. Autocb still remains a variable, and can be still passed around. It just serves as a marker, not a keyword.
